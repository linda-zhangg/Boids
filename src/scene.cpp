...
void Scene::loadCore() {
    ...
    neighbourDist = 5.0;
    for(int i=0; i<200; i++){
        m_boids.push_back(Boid(linearRand(vec3(-1), vec3(1)), sphericalRand(1.0), 0));
    }
}
...
void Scene::loadCompletion() {
    ...
    for(int i=0; i<100; i++){
        m_boids.push_back(Boid(linearRand(vec3(-1), vec3(1)), sphericalRand(1.0), 1));
    }
    //predators
    for(int i=0; i<2; i++){
        m_boids.push_back(Boid(linearRand(vec3(-1), vec3(1)), sphericalRand(1.0), 2));
    }
}
...
void Scene::loadChallenge() {
    ...
    //obstacles
    m_boids.push_back(Boid(vec3(10), vec3(0.0), 3));
    m_boids.push_back(Boid(vec3(5,-10,-10), vec3(0.0), 3));
    m_boids.push_back(Boid(vec3(-10,5,-5), vec3(0.0), 3));
...
void Scene::renderGUI() {
    ...
    ImGui::Separator();
    ImGui::Text("Speeds");
    ImGui::SliderFloat("min speed", &minSpeed, 1, 5.9);
    ImGui::SliderFloat("max speed", &maxSpeed, 6, 10);
    ImGui::SliderFloat("max accel", &maxAccel, 0.1, 5);
    
    ImGui::Separator();
    ImGui::Text("Boundary");
	ImGui::SliderFloat3("Bound hsize", value_ptr(m_bound_hsize), 0, 100.0, "%.0f");
    ImGui::Combo("Mode", &selectedMode, modes, 3); //0 = wrap, 1 = bounce, 2 = soft
    if(selectedMode == 2){
        ImGui::SliderFloat("soft weight", &softWeight, 0.1, 2);
    }
    
    ImGui::Separator();
    ImGui::Text("Boid Parameters");
    ImGui::SliderFloat("NeighbourDist", &neighbourDist, 1.0, 30.0);
    ImGui::SliderFloat("Avoidance", &avoidWeight, 0.1, 10.0);
    ImGui::SliderFloat("Cohesion", &cohesionWeight, 0.1, 10.0);
    ImGui::SliderFloat("Alignment", &alignWeight, 0.1, 10.0);
    ImGui::SliderFloat("AntiPredator", &antiPredator, 1.0, 100.0);

    ImGui::Separator();
    ImGui::Text("Predator Parameters");
    ImGui::SliderFloat("p.min speed", &pMinSpeed, 1, 5.9);
    ImGui::SliderFloat("p.max speed", &pMaxSpeed, 6.0, 10.0);
    ImGui::SliderFloat("p.max accel", &pMaxAccel, 0.1, 5.0);
    ImGui::SliderFloat("p.NeighbourDist", &pNeighbourDist, 0.0, 100.0);
    ImGui::SliderFloat("p.Avoidance", &pAvoidWeight, 1.0, 10.0);
    ImGui::SliderFloat("Chasing", &pChaseWeight, 1.0, 10.0);
